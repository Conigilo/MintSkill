  ...
  ({ user, onRequestEndorsement, onExport }: ProfileCardProps) => {
    return (
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-5 text-center">
        ...
        <div className="font-serif text-xl font-semibold tracking-tight leading-tight">
          {user.displayName || 'User'}
        </div>
        <div className="font-mono text-xs text-[var(--muted)] mb-1">
          @{user.email?.split('@')[0] || 'user'}
        </div>
        <div className="text-sm font-medium text-[var(--accent)] mb-3">
          Full-Stack Developer
        </div>
        
        <div className="text-sm text-[var(--muted)] text-left mb-3.5 leading-relaxed">
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold bg-[var(--accent)] text-[#0d1117] border border-[var(--accent)] hover:bg-[#79c0ff] transition-colors"
        >
          ✉ Request Endorsement
        </button>
        <button
          onClick={onExport}
          className="w-full mt-1.5 inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold bg-transparent text-[var(--text)] border border-[var(--border)] hover:bg-[var(--surface2)] transition-colors inline-block"
        >
          ⬇ Export Portfolio
        </button>
      </div>
