...
export default function ProfileView({ username, initialPortfolio }: ProfileViewProps) {
    ...
    <div className="min-h-screen bg-slate-50 text-[#c9d1d9] font-sans selection:bg-purple-500/30">
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-md bg-slate-50/80 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <button
            onClick={() => router.push("/explore")}
            className="group text-slate-500 hover:text-slate-900 flex items-center gap-2 text-sm transition-all cursor-pointer"
          >
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={handleShare}
              className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-slate-900 px-4 py-1.5 rounded-full border border-white/10 text-sm transition-all cursor-pointer"
            >
            ...
            <div className="sticky top-28 space-y-8">
              ...
              {(!user || user.uid !== p.id) && (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-slate-900 font-bold py-3 rounded-2xl shadow-[0_0_20px_rgba(168,85,247,0.2)] transition-all transform hover:-translate-y-1 active:translate-y-0 cursor-pointer"
                >
                  Endorse Skills
                </button>
              )}
