            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {developers.map(dev => (
                        <div 
                            key={dev.id}
                            onClick={() => router.push(`/profile/${dev.username || dev.id}`)}
                            className="bg-[#161b22] border border-gray-800 p-6 rounded-2xl hover:border-gray-600 transition-all cursor-pointer hover:shadow-lg hover:shadow-blue-500/10"
                        >
                            <div className="flex gap-4 items-center mb-4">
                                {dev.photoURL ? (
                                    ...
                                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center font-bold text-lg">
                                        {(dev.displayName || 'D').charAt(0).toUpperCase()}
                                    </div>
                                )}
                                <div>
                                    <h3 className="font-bold">{dev.displayName || 'Developer'}</h3>
